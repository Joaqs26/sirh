...
SELECT  
    Entradas.id_tbl_empleados_hraes,
    NULL AS observaciones,
    TRUE AS es_por_retardo,                -- Entrada después de Hora Máxima de Retardo
...
FROM (
    ...
    ) -- Después de hora máxima de retardo
) AS Entradas
WHERE NOT EXISTS (
    -- Evita duplicados si ya hay una falta registrada
    SELECT 1
    FROM central.ctrl_faltas f
...
AND NOT EXISTS (
    -- Excluye si ya está justificado en la tabla temporal
    SELECT 1
    FROM central.masivo_ctrl_temp_faltas_just mj
    WHERE mj.rfc = (
        SELECT e.rfc 
    ...
    {
        $query = pg_query(" INSERT INTO central.ctrl_faltas (id_tbl_empleados_hraes,
                                                            observaciones,
                                                            es_por_retardo,
...
FROM (
    ...
    WHERE ca.fecha NOT IN (
      ...
      AND ai.id_cat_asistencia_estatus = 1   
      AND ai.id_cat_asistencia_config = 1
      AND ai.no_dispositivo IS NOT NULL -- ⚠️ Filtro agregado
    GROUP BY ca.fecha, ca.id_tbl_empleados_hraes
) AS Entradas
WHERE Entradas.registros = 1
  AND Entradas.fecha NOT IN ('2024-12-24', '2024-12-31', '2024-01-10')
  AND NOT EXISTS (
        SELECT 1
        FROM central.ctrl_faltas cf
